...
def extract_text_from_docx(file_path):
    ...
    that appears in both doc.paragraphs and doc.tables.
    """
    doc = Document(file_path)

    # Collect table-cell paragraph objects to avoid duplicating them
...
def role_relevance_score(role, job_keywords):
    # Use dict key access ["text"] — bullets are dicts, not docx objects
...
async def analyze_resume(file: UploadFile = File(...)):
    ...
    Temp file is always cleaned up after the request.
    """
    suffix = ".docx"
    tmp_path = f"analyze_{uuid.uuid4().hex}{suffix}"
...
async def match_job(data: MatchJobRequest):
    ...
    Returns a 0-100 match score plus matching and missing keywords.
    """
...
async def debug_parse(file: UploadFile = File(...)):
    """
    Parse an uploaded resume and return the structured model.
    Useful during development to inspect parser output.
    Temp file is always cleaned up after the request.
    """
    import shutil
...
async def tailor_resume_docx_preserve(
    ...
    """
    Accept a .docx resume and a job description, rewrite the resume content
    to better match the job, and return the tailored .docx file.

    Original formatting (fonts, bold, italic, colors) is preserved.
    ...
    try:
        ...
        # Single Document instance flows through parse -> tailor -> save
        ...
        tailor_engine = ResumeTailorEngine(
            ...
            paragraph_refs=parser._paragraph_refs,   # live docx objects
        ...
        # NOTE: FileResponse streams the file then the finally block deletes it.
        # background parameter handles cleanup after the response is sent.
        ...
        # If something goes wrong before we return FileResponse,
